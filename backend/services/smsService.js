...
const sendIssueStatusSMS = async (tenant, issue) => {
  ...
  await sendSMS(tenant.phone, `Appoint Eiendom: Your issue "${issue.title}" status has been updated to ${statusText}. Log in to the portal for details.`);
};

const sendAnnouncementSMS = async (tenants, title) => {
  for (const tenant of tenants) {
    if (tenant.phone) {
      await sendSMS(tenant.phone, `Appoint Eiendom: New announcement — "${title}". Log in to the portal to read more.`);
    }
  }
};
...
const sendDocumentSMS = async (tenant, title) => {
  if (!tenant?.phone) return;
  await sendSMS(tenant.phone, `Appoint Eiendom: A new document "${title}" has been shared with you. Log in to the portal to view it.`);
};
