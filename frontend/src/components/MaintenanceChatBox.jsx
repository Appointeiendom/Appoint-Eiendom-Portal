...
export default function MaintenanceChatBox({ issueId, maintenanceId, maintenanceName }) {
  ...
  useEffect(() => {
    ...
    const onMessage = (msg) => {
      if (String(msg.maintenanceId) === String(maintenanceId)) {
  ...
  const sendMessage = (e) => {
    ...
    const socket = getSocket();
    if (socket) {
      socket.emit('send_message', { issueId, message: input.trim(), maintenanceId });
    }
    setInput('');
  };
  ...
  const sendQuote = () => {
    ...
    const socket = getSocket();
    const msg = quoteNote.trim()
      ? `Quote: ${quoteAmount} kr\n${quoteNote.trim()}`
      : `Quote: ${quoteAmount} kr`;
    if (socket) {
  ...
  if (loading) return <div className="text-center py-8 text-gray-400 text-sm">Loading chat...</div>;
  ...
  return (
    <div className="flex flex-col h-96 border border-gray-200 rounded-xl overflow-hidden bg-white">
      <div className="bg-amber-50 px-4 py-3 border-b border-amber-100 flex items-center justify-between">
        <div>
          <h3 className="font-semibold text-gray-700 text-sm">Chat with {maintenanceName}</h3>
          <p className="text-xs text-gray-400">Maintenance thread</p>
        </div>
        {isMaintenance && (
          <button
      ...
      {/* Quote form (maintenance only) */}
      {showQuoteForm && isMaintenance && (
        <div className="bg-amber-50 border-b border-amber-100 p-3 space-y-2">
          <p className="text-xs font-medium text-amber-700">Send a price quote to the tenant</p>
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => setShowQuoteForm(false)} className="text-xs text-gray-500 hover:text-gray-700">
              Cancel
            </button>
          </div>
        </div>
              ...
              <div className={`max-w-xs lg:max-w-md flex flex-col ${isOwn ? 'items-end' : 'items-start'}`}>
                ...
                {isQuote ? (
                  <div className={`rounded-2xl overflow-hidden border-2 ${isOwn ? 'border-amber-400' : 'border-amber-300'}`}>
                    <div className="bg-amber-500 text-white px-4 py-2 text-xs font-semibold uppercase tracking-wide">
                      💰 Price Quote
                    </div>
                  ...
                  <div className={`px-4 py-2 rounded-2xl text-sm ${
                    isOwn ? 'bg-emerald-500 text-white rounded-tr-sm' : 'bg-gray-100 text-gray-800 rounded-tl-sm'
                  }`}>
                    {msg.message}
                  </div>
                )}
