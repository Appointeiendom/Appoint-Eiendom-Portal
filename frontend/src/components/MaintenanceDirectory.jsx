...
export default function MaintenanceDirectory({ issue }) {
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-gray-500">{t('maintenance.directoryBody')(issue.category)}</p>
      {workers.map(worker => (
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    ...
                    <span className="inline-block text-xs bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full mt-0.5">{t(`categories.${worker.trade}`)}</span>
                  </div>
                  <button onClick={() => setOpenChat(openChat === worker._id ? null : worker._id)}
